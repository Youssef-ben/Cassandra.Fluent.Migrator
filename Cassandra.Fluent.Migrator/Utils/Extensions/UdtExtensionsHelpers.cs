namespace Cassandra.Fluent.Migrator.Utils.Extensions
{
    ...
    internal static class UdtExtensionsHelpers
    {
        ...
        internal static async Task<ICassandraFluentMigrator> ExecuteBuildUdtAsync<TEntity>(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string name, bool shouldBeFrozen)
                where TEntity : class
        {
            ...
            var query = new StringBuilder(UdtCqlStatements.UDT_CREATE_STATEMENT.NormalizeString(name));
        ...
        /// <returns>The Cassandra CQL query.</returns>
        /// <exception cref="NullReferenceException">Thrown when the arguments are empty or null.</exception>
        internal static async Task<ICassandraFluentMigrator> ExecuteDeleteUdtAsync<TEntity>(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string name)
                where TEntity : class
        {
            Check.NotNull(self, "The argument [cassandra fluent migrator]");
            Check.NotEmptyNotNull(name, $"The argument [{nameof(name)}]");

            var query = UdtCqlStatements.UDT_DROP_STATEMENT.NormalizeString(name);
        ...
        /// <returns>The Cassandra CQL query.</returns>
        /// <exception cref="NullReferenceException">Thrown when the arguments are empty or null.</exception>
        internal static async Task<ICassandraFluentMigrator> ExecuteAlterUdtAddColumnQuery(
                ...
                [NotNull] string type)
        {
            ...
            var query = UdtCqlStatements.UDT_ADD_COLUMN_STATEMENT.NormalizeString(udt, column, type);

            return await self.ExecuteUdtStatementAsync(query,
                           AppErrorsMessages.UDT_COLUMN_TYPE_EXISTS.NormalizeString(column));
        }
        ...
        /// <returns>The Cassandra CQL query.</returns>
        /// <exception cref="NullReferenceException">Thrown when the arguments are empty or null.</exception>
        internal static async Task<ICassandraFluentMigrator> ExecuteAlterUdtRenameColumnQuery(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string udt, [NotNull] string column,
                [NotNull] string target)
        {
            ...
            var query = UdtCqlStatements.UDT_RENAME_COLUMN_STATEMENT.NormalizeString(udt, column, target);

            return await self.ExecuteUdtStatementAsync(query,
                           AppErrorsMessages.COLUMN_TYPE_NOT_FOUND.NormalizeString(column));
        }

        private static async Task<ICassandraFluentMigrator> ExecuteUdtStatementAsync(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string query,
                [NotNull] string errorMessage = "throw error")
        {
            try
            {

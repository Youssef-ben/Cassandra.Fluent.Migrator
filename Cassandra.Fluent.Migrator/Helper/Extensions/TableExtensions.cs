namespace Cassandra.Fluent.Migrator.Helper
{
    ...
    /// <summary>
    ///     This partial Extensions class is used to group all the Cassandra {Table} Actions for the {Cassandra Fluent
    ///     Migrator}.
    /// </summary>
    public static partial class CfmExtensions
    {
        ...
        public static async Task<ICassandraFluentMigrator> AddColumnAsync([NotNull] this ICassandraFluentMigrator self,
                [NotNull] string table, [NotNull] string column, [NotNull] Type type, bool shouldBeFrozen = false)
        {
        ...
        /// <exception cref="NullReferenceException">Thrown when one or all the specified arguments are invalid or null.</exception>
        /// <exception cref="ObjectNotFoundException">Thrown when the table doesn't exists.</exception>
        public static async Task<ICassandraFluentMigrator> AddColumnAsync<Table>(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string table, [NotNull] string column,
        ...
        ///     Thrown when the Column is not a primary key or the target column name
        ///     already exists.
        /// </exception>
        public static async Task<ICassandraFluentMigrator> RenamePrimaryColumnAsync(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string table, [NotNull] string old,
                [NotNull] string target)
        {
            ...
            {
                throw new InvalidOperationException(AppErrorsMessages.COLUMN_TYPE_EXISTS.NormalizeString(target));
            }

            if (!self.IsPrimaryKey(table, old))
            {
                throw new InvalidOperationException(
                        AppErrorsMessages.CAN_NOT_RENAME_NONE_PRIMARY_KEY.NormalizeString(old));
            }
        ...
        /// <exception cref="NullReferenceException">Thrown when one or all the specified arguments are invalid or null.</exception>
        /// <exception cref="ObjectNotFoundException">Thrown when the table doesn't exists.</exception>
        public static async Task<ICassandraFluentMigrator> DropColumnAsync(this ICassandraFluentMigrator self,
                [NotNull] string table, [NotNull] string column)
        {
            Check.NotNull(self, "The argument [cassandra fluent migrator object]");
            Check.NotEmptyNotNull(table, $"The argument [{nameof(table)}]");
            Check.NotEmptyNotNull(column, $"The argument [{nameof(column)}]");
        ...
        private static async Task<ICassandraFluentMigrator> ExecuteAddColumnAsync(
                [NotNull] this ICassandraFluentMigrator self, [NotNull] string table, [NotNull] string column,
                [NotNull] string type)
        {
            table = table.NormalizeString();
